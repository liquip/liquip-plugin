...
public class BlockEventListener implements Listener {
    ...
    public void onBreak(@NotNull BlockBreakEvent event) {
        ...
        if (parsedIdentifier.isEmpty()) {
            Liquip.getProvidingPlugin(Liquip.class).getSLF4JLogger().error("{BlockBreakEvent} Item with invalid " + "identifier");
            return;
        }
        ...
        if (liquipItem == null) {
            Liquip.getProvidingPlugin(Liquip.class).getSLF4JLogger().error("{BlockBreakEvent} Item with unknown identifier");
            return;
        }
    ...
    public void onPlace(@NotNull BlockPlaceEvent event) {
        ...
        final Optional<Identifier> parsedIdentifier = Identifier.parse(identifierString);
        if (parsedIdentifier.isEmpty()) {
            Liquip.getProvidingPlugin(Liquip.class).getSLF4JLogger().error("{BlockPlaceEvent} Item with invalid identifier");
            return;
        }
        final Identifier identifier = parsedIdentifier.get();
        final LiquipItem liquipItem = Liquip.getProvider().getItemRegistry().get(identifier);
        if (liquipItem == null) {
            Liquip.getProvidingPlugin(Liquip.class).getSLF4JLogger().error("{BlockPlaceEvent} Item with unknown identifier");
            return;
        }
        liquipItem.callEvent(BlockPlaceEvent.class, event);
    }
}
